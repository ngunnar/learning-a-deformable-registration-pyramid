...
class DeformableConvLayer(Conv3D):
    ...
    def build(self, input_shape):
        ...
        offset_num = self.kernel_size[0] * self.kernel_size[1] * self.kernel_size[2] * self.num_deformable_group
        self.offset_layer_kernel = self.add_weight(
            name='offset_layer_kernel',
            shape=self.kernel_size + (input_dim, offset_num * 3),  # 3 means x,y and z axis
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        in_d, in_h, in_w = [int(i) for i in inputs.get_shape()[1: 4]]  # input feature map size
        out_d, out_h, out_w = [int(i) for i in offset.get_shape()[1: 4]]  # output feature map size
        filter_h, filter_w = self.kernel_size

        # get x, y axis offset
        offset = tf.reshape(offset, [batch_size, out_d, out_h, out_w, -1, 2])
        z_off, y_off, x_off = offset[:, :, :, :, 0], offset[:, :, :, :, 1],  offset[:, :, :, :, 2]

        # input feature map gird coordinates
        z, y, x = self._get_conv_indices([in_d, in_h, in_w])
        z, y, x = [tf.expand_dims(i, axis=-1) for i in [z, y, x]]
        z, y, x = [tf.tile(i, [batch_size, 1, 1, 1, 1, self.num_deformable_group]) for i in [z, y, x]]
        z, y, x = [tf.reshape(i, [*i.shape[0: 4], -1]) for i in [z, y, x]]
        z, y, x = [tf.to_float(i) for i in [x, y, x]]

        # add offset
        z, y, x = z + z_off, y + y_off, x + x_off
        z = tf.clip_by_value(z, 0, in_d - 1)
        y = tf.clip_by_value(y, 0, in_h - 1)
        x = tf.clip_by_value(x, 0, in_w - 1)

        # get four coordinates of points around (x, y)
        z0, y0, x0 = [tf.to_int32(tf.floor(i)) for i in [z, y, x]]
        z1, y1, x1 = z0 + 1, y0 + 1, x0 + 1
        # clip
        z0, y0, y1 = [tf.clip_by_value(i, 0, in_h - 1) for i in [z0, y0, y1]]
        z0, x0, x1 = [tf.clip_by_value(i, 0, in_w - 1) for i in [z0, x0, x1]]

        # get pixel values
        indices = [[z0, y0, x0], [z0, y0, x1], [z0, y1, x0], [z0, y1, x1], [z1, y0, x0], [z1, y0, x1], [z1, y1, x0], [z1, y1, x1]]
        p0, p1, p2, p3, p4, p5, p6, p7 = [DeformableConvLayer._get_pixel_values_at_point(inputs, i) for i in indices]

        # cast to float
        z0, z1, x0, x1, y0, y1 = [tf.to_float(i) for i in [z0, z1, x0, x1, y0, y1]]
        # weights
        ############# HERE ###################
        w0 = (y1 - y) * (x1 - x)
        w1 = (y1 - y) * (x - x0)
